(function ($, Drupal, window, document, undefined) {
  ...
  function Prettify(options) {
    ...
    var view = {
      ...
      init: function (options) {
        ...
        view.count = 0;
        view.emptyText = Drupal.t('No file selected');

        if (!view.item.hasClass('prettified')) {
      ...
      _renderFile: function () {
        ...
        var fake = $('<div><span><a class="button">' + Drupal.t('Browse') + '</a></span><span class="filetext">' + view.emptyText + '</span></div>');
        ...
        if (Drupal.settings.prettify.add_remove) {
          view.clear = $('<a class="button fileclear">' + Drupal.t('Remove') + '</a>');
      ...
      _change: function (event) {
        ...
        if (view.type == "fileinput") {
          ...
          if (value) {
            ...
            if (!Drupal.settings.prettify.hide_upload) {
              view.submit.show();
            }
          }
          else {
  ...
  Drupal.behaviors.prettify = {
    attach: function (context, settings) {
      $('select, input[type="file"]', context).each(function () {
        new Prettify({el: this});
      });
    }
  };
